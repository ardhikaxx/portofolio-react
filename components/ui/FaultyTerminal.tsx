...
void main() {
    ...
    if(uChromaticAberration != 0.0){
      vec2 ca = vec2(uChromaticAberration) * 0.01 / iResolution.xy;
...
export default function FaultyTerminal({
    ...
    // Get DPR safely with useEffect
    const safeDpr = useMemo(() => {
        if (dpr !== undefined) return dpr;
        if (typeof window !== 'undefined') {
            return Math.min(window.devicePixelRatio || 1, 2);
        }
        return 1; // Default for server-side
    }, [dpr]);
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        ...
        mouseRef.current = { x, y: 1 - y }; // Flip Y coordinate for GL space
    }, []);
    ...
    useEffect(() => {
        ...
        const update = (t: number) => {
            ...
            if (mouseReact) {
                const dampingFactor = 0.15; // Increased for smoother follow
                ...
                program.uniforms.uMouse.value = mouseUniform;
            }
        ...
        if (mouseReact) {
            ...
            ctn.style.cursor = 'none'; // Hide cursor for better effect
        }
